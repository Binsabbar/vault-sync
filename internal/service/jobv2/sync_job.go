...
func (job *SyncJob) executeDelete(ctx context.Context) (*SyncJobResult, error) {
	...
	for _, deleteResult := range deleteResults {
		...
		if dbErr := job.databaseClient.DeleteSyncedSecret(job.mount, job.keyPath, deleteResult.DestinationCluster); dbErr != nil && status != SyncJobStatusDeleted {
			logger.Error().
				Str("cluster", deleteResult.DestinationCluster).
				Err(dbErr).
